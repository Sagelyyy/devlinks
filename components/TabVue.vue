...
function getState(state: string) {
  switch (state) {
    case 'active':
      return 'bg-pale-blurple text-blurple'
    default:
      return 'bg-transparent text-dark-grey hover:text-blurple'
  }
}
</script>
...
<template>
  <div
    :class="getState(props.state)"
    class="w-[138px] h-[46px] transition-colors text-center font-instrument-bold flex justify-center gap-2 cursor-pointer"
  >
    <svg
      class="self-center"
    ...
    <span class="self-center"><slot></slot></span>
  </div>
</template>
